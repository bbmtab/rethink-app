...
object WireguardManager : KoinComponent {
    ...
    // Bridge: upstream Boolean refactor kept; Plus lockdown health-gate
    // preserved (unhealthy lockdown config => excluded => caller
    // lockdown-override adds Block). suspend retained for getProxyStatusById.
    private suspend fun canUseConfig(idStr: String, type: String, usesMobileNw: Boolean, ssid: String): Boolean {
        if (idStr.isEmpty()) {
            return false
        }
        ...
        if (lockdown && !isHealthy) {
            Logger.d(LOG_TAG_PROXY, "lockdown wg for $type is inactive/unhealthy => exclude")
            return false
        }
