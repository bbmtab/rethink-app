...
class InspectionAppPolicyController(
    ...
    private val systemHardBypassUids: Set<Int> = emptySet()
) {
    private val normalizedSystemHardBypassPackages =
    ...
    fun stateFor(
        ...
        if (
            normalizedPackage in normalizedSystemHardBypassPackages ||
                uid?.let(systemHardBypassUids::contains) == true
        ) {
    ...
    fun setInspectionEnabled(
        ...
        when (tier) {
            InspectionAppPolicyTier.SYSTEM_BYPASS -> return
